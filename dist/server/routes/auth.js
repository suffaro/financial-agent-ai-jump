...
router.get('/google/callback', passport.authenticate('google', { failureRedirect: '/login' }), async (req, res) => {
    try {
        const user = req.user;
        const token = jwt.sign({ userId: user.id, email: user.email }, process.env['JWT_SECRET'] || 'fallback-secret', { expiresIn: '7d' });
...
router.get('/hubspot', (req, res) => {
    ...
    if (!token) {
        ...
        try {
            const decoded = jwt.verify(frontendToken, process.env['JWT_SECRET'] || 'fallback-secret');
            req.session.user = { id: decoded.userId, email: decoded.email };
...
router.get('/hubspot/callback', async (req, res) => {
    try {
        ...
        if (state) {
            try {
                const userToken = Buffer.from(state, 'base64').toString();
                const decoded = jwt.verify(userToken, process.env['JWT_SECRET'] || 'fallback-secret');
        ...
        const redirectUrl = `${baseUrl}/integrations?hubspot=connected`;
        res.redirect(redirectUrl);
    }
    catch (error) {
