...
router.post('/hubspot/notes/sync', requireHubspotAuth, async (req, res) => {
    try {
        await hubspotService.syncNotes(req.user.id);
...
router.get('/summary', async (req, res) => {
    try {
        const [emailCount, calendarCount, contactCount, noteCount] = await Promise.all([
            prisma.emailMessage.count({ where: { userId: req.user.id } }),
            prisma.calendarEvent.count({ where: { userId: req.user.id } }),
            prisma.hubspotContact.count({ where: { userId: req.user.id } }),
            prisma.hubspotNote.count({ where: { userId: req.user.id } })
        ]);
        res.json({
            summary: {
                emails: emailCount,
...
router.get('/gmail/search', requireGoogleAuth, async (req, res) => {
    try {
        ...
        const emails = await googleService.searchEmails(req.user.id, query);
        res.json({ emails });
    }
    catch (error) {
...
router.get('/hubspot/search', requireHubspotAuth, async (req, res) => {
    try {
        ...
        const contacts = await hubspotService.searchContacts(req.user.id, query);
        res.json({ contacts });
    }
    catch (error) {
