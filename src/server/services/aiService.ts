...
const groqRateLimiter = RateLimiter.getInstance('groq', 5, 1000);
...
export class AIService {
    async processMessage(userId: string, conversationId: string, content: string, context: string = 'all') {
        try {
            ...
            if (toolCalls.length > 0) {
                ...
                if (toolResponses.length > 0) {
                    ...
                    const followUpMessages = [
                        ...messages,
                        { role: 'assistant' as const, content: aiResponse, tool_calls: toolCalls },
                    ...
                    try {
                        await groqRateLimiter.checkLimit();
    ...
    private buildContext(user: any, emails: any[], calendarEvents: any[], contacts: any[], tasks: any[], relevantDocs: RelevantDocument[] = []) {
        let context = `User: ${user.name || user.email}\n\n`;

        if (user.ongoingInstructions.length > 0) {
            context += 'Ongoing Instructions:\n';
            user.ongoingInstructions.forEach((instruction: any) => {
    ...
    async addDocumentToVectorStore(userId: string, content: string, metadata: any): Promise<void> {
        try {
    ...
    async searchVectorStore(userId: string, query: string, limit: number = 5, contextFilter: string = 'all'): Promise<RelevantDocument[]> {
        try {
            ...
            if (contextFilter === 'emails') {
                ...
                filteredDocuments = documents.filter(doc =>
                    doc.metadata?.source === 'hubspot_contact' || doc.metadata?.source === 'hubspot_note'
                );
            }
            ...
            try {
                ...
                let filteredDocuments = documents;
                if (contextFilter === 'emails') {
                    filteredDocuments = documents.filter(doc => (doc.metadata as any)?.source === 'email');
                } else if (contextFilter === 'calendar') {
                    filteredDocuments = documents.filter(doc => (doc.metadata as any)?.source === 'calendar');
                } else if (contextFilter === 'contacts') {
                    filteredDocuments = documents.filter(doc =>
                        (doc.metadata as any)?.source === 'hubspot_contact' || (doc.metadata as any)?.source === 'hubspot_note'
                    );
                }
    ...
    async executeToolCalls(userId: string, toolCalls: any[]): Promise<any[]> {
        const results: any[] = [];
    ...
    private async handleSearchEmails(userId: string, args: { query: string }) {
   ...
   Preview: ${snippet}${snippet.length > 0 ? '...' : ''}`;
        ...
        return {
            ...
            vectorMatches: emailVectorResults.length > 0 ? emailVectorResults.map(doc => ({
                ...
                from: (doc.metadata as any).from
            })) : undefined
        };
    }

    private async handleSearchContacts(userId: string, args: { query: string }) {
        ...
        const contacts = await prisma.hubspotContact.findMany({
            where: {
                ...
                OR: [
                    ...
                    ...searchTerms.map(term => ({ firstName: { contains: term, mode: 'insensitive' as const } })),
                    ...searchTerms.map(term => ({ lastName: { contains: term, mode: 'insensitive' as const } }))
                ]
            },
            take: 15,
            orderBy: { createdAt: 'desc' }
        ...
        const uniqueEmailContacts = emailContacts.reduce((acc: any[], email) => {
        ...
        const contactVectorResults = vectorResults.filter(doc =>
            doc.metadata.source === 'hubspot_contact' || doc.metadata.source === 'hubspot_note'
        );
        ...
        if (uniqueEmailContacts.length > 0) {
            if (formattedResponse.length > 0) formattedResponse.push('');
        ...
        return {
            ...
            vectorMatches: contactVectorResults.length > 0 ? contactVectorResults.map(doc => ({
                ...
                contactName: (doc.metadata as any).contactName
            })) : undefined
        };
    }

    private async handleScheduleAppointment(userId: string, args: { contactName: string, duration?: number, description?: string, specificTime?: string }) {
        try {
...
Best regards`;
            ...
            const appointmentTask = allTasks.find(task =>
                task.metadata?.type === 'multi_step_parent' &&
    ...
    private async handleSendEmail(userId: string, args: { to: string, subject: string, body: string }) {
        try {
    ...
    private async handleCreateTask(userId: string, args: { title: string, description?: string, priority?: string }) {
    ...
    private async handleAddOngoingInstruction(userId: string, args: { instruction: string }) {
        try {
    ...
    private async handleSendEmailToContact(userId: string, args: { contactName: string, subject: string, body: string }) {
        try {
    ...
    private async handleCreateHubspotContact(userId: string, args: { email: string, firstName?: string, lastName?: string, company?: string, note?: string }) {
        try {
    ...
    async processWebhookWithInstructions(userId: string, webhookSource: string, webhookData: any): Promise<void> {
        try {
...
If any action should be taken based on these instructions, use the available tools to execute them.`;
    ...
    private async handleSearchCalendar(userId: string, args: { query: string, startDate?: string, endDate?: string }) {
        try {
            const query = args.query.toLowerCase();
            let dateFilter: any = {};
    ...
    private async handleCreateCalendarEvent(userId: string, args: {
        title: string,
        description?: string,
        startTime: string,
        endTime: string,
        ...
        try {
            ...
            if (!args.attendees || args.attendees.length === 0) {
                ...
                const isPersonalEvent = personalKeywords.some(keyword =>
                    args.title.toLowerCase().includes(keyword) ||
    ...
    private async handleDeleteCalendarEvents(userId: string, args: {
        query: string,
        startDate?: string,
        endDate?: string
    }) {
    ...
    private async handleProcessAppointmentResponse(userId: string, args: {
        contactName: string,
        responseType: 'accepted' | 'declined' | 'alternative_time' | 'unclear',
        selectedTime?: string,
        ...
        try {
            ...
            const appointmentTask = waitingTasks.find(task =>
                task.metadata?.type === 'wait_response' &&
    ...
    private async handleListAllContacts(userId: string, args: { limit?: number }) {
