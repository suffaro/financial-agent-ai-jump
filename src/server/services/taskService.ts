...
export class TaskService {
    async createTask(userId: string, params: TaskParams): Promise<any> {
        try {
    ...
    async getTaskById(userId: string, taskId: string): Promise<any> {
        try {
    ...
    async updateTask(userId: string, taskId: string, updates: Partial<TaskParams & { status: string; completedAt?: string }>): Promise<any> {
        try {
            ...
            if (updates.parentTaskId !== undefined) updateData.parentTaskId = updates.parentTaskId;
            if (updates.stepOrder !== undefined) updateData.stepOrder = updates.stepOrder;
    ...
    async createMultiStepTask(userId: string, params: {
        title: string;
        description?: string;
        priority?: 'low' | 'medium' | 'high';
        steps: Array<{ title: string; description?: string; metadata?: any }>;
        metadata?: any;
    }): Promise<any> {
        try {
            // Create parent task
    ...
    async getTaskWithSteps(userId: string, taskId: string): Promise<any> {
        try {
            const task = await prisma.task.findFirst({
                where: {
                    id: taskId,
    ...
    async advanceToNextStep(userId: string, taskId: string): Promise<any> {
        try {
            ...
            const nextStep = parentTask.subTasks.find((step: any) => 
                step.stepOrder === (task.stepOrder || 0) + 1 && step.status === 'pending'
            );
            ...
            if (nextStep) {
                ...
                // All steps completed, mark parent as completed
    ...
    async getWaitingTasks(userId: string): Promise<any[]> {
        try {
            const tasks = await prisma.task.findMany({
                where: {
                    userId,
    ...
    async resumeWaitingTask(userId: string, taskId: string, responseData: any): Promise<any> {
        try {
            const task = await this.getTaskWithSteps(userId, taskId);
            ...
            // Update task metadata with response data
