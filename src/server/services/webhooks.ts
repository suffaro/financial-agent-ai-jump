...
export async function areWebhooksEnabled(userId: string): Promise<boolean> {
    try {
 ...
 * - GOOGLE_PROJECT_ID: Your Google Cloud Project ID (for Gmail Push notifications)
 * - BASE_URL: Your publicly accessible domain (e.g., https://yourdomain.com)
 * 
 * Google Cloud Setup:
 * 1. Enable Gmail and Calendar APIs
 * 2. Create a Pub/Sub topic named "gmail-notifications"
 * 3. Configure domain verification for webhook URLs
 * 
 * HubSpot Setup:
 * 1. Get your HubSpot Portal ID from developer settings
 * 2. Configure webhook subscriptions in HubSpot developer portal
 * 3. Update the setupHubspotWebhook function with correct portal ID
 */
...
export async function setupGmailWebhook(accessToken: string, userId: string) {
    try {
...
export async function setupCalendarWebhook(accessToken: string, userId: string) {
    try {
...
export async function setupHubspotWebhook(accessToken: string, userId: string) {
    try {
        ...
        /*
        const response = await fetch('https://api.hubapi.com/webhooks/v1/your-portal-id/subscriptions', {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            throw new Error(`HubSpot webhook setup failed: ${response.statusText}`);
        }

        console.log('HubSpot webhook set up successfully');
...
async function processGmailWebhookEvent(event: any) {
...
async function processCalendarWebhookEvent(event: any) {
