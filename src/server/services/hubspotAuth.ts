...
export function initializeHubspotAuth(passportInstance: passport.Authenticator) {
    passportInstance.use('hubspot', new OAuth2Strategy({
        ...
        try {
            // For HubSpot, we need to get user info from the access token
            const response = await fetch('https://api.hubapi.com/oauth/v1/access-tokens/' + accessToken);
            const userInfo = await response.json();

            const email = (userInfo as any).user;
